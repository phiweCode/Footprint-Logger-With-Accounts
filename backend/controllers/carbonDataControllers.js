const { getCH4GWPData, getData, getInfo, getMetadata, getHundredYearGWPS, getRegionClassification, getSectorClassification } = require('../services/carbonDataServices')  
...
const getSectorClassificationController = async (req, res) => { 
    ...
    const results= await getSectorClassification() 
        if(!results) return res.status(200).json({ 
        message: 'No data found!'
    })
